...
const ACCENT   = '#C4854A'
const INACTIVE = 'rgba(26,26,26,0.35)'
      ...
      {navItems.map(({ href, icon: Icon, label, exact }) => {
        ...
        return (
          <Link key={href} href={href}
            className="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C4854A]/30"
            style={{ background: active ? '#F4F1EC' : undefined, color: active ? '#1A1A1A' : INACTIVE }}>
    ...
    <div>
      ...
      {open && (
        <div
          ...
          style={{ height: 56, borderBottom: '1px solid #E8E4DC' }}
        >
          {allMore.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link
                ...
                className="flex-1 flex flex-col items-center justify-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C4854A]/30"
              ...
              >
                ...
                <span className="font-semibold text-center w-full truncate"
      ...
      <div className="flex" style={{ height: 56 }}>
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C4854A]/30"
          style={{ WebkitTapHighlightColor: 'transparent', touchAction: 'manipulation', minWidth: 0, background: 'none', border: 'none', cursor: 'pointer' }}
        >
          {open
            ? <X size={22} strokeWidth={2.5} aria-hidden="true" style={{ color: ACCENT }} />
            : <MoreHorizontal size={22} aria-hidden="true" strokeWidth={anyMoreActive ? 2.5 : 1.8}

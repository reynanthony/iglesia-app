...
export default async function SalaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="room-height flex flex-col">
      {/* Header */}
      <div className="flex-shrink-0 px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #0D3352' }}>
        ...
        {isCreator && (
          <form action={closeRoom.bind(null, room.id)}>
            <button
              type="submit"
              className="text-xs text-red-400 hover:text-red-300 border border-red-400/30 hover:border-red-400/60 px-4 py-3 rounded-lg transition"
            >
              Cerrar sala
            </button>
          </form>

...
import { Home, MessageCircle, Users, LogOut, PlusSquare, User } from 'lucide-react'
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { href: '/app/oracion', icon: Users, label: 'Oración' },
      ...
      <aside className="hidden md:flex w-64 border-r border-slate-800 flex-col fixed h-full z-30">
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-500 font-bold text-sm overflow-hidden">
              {profile?.avatar_url ? (
        ...
        <div className="flex items-center justify-around px-2 py-2">
          {navItems.map(({ href, icon: Icon, label }) => (
            <Link
              key={href}
              href={href}
              className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl text-slate-400 hover:text-amber-400 transition"
            >
              <Icon size={22} />

...
export default async function SeriesDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const color = series.cover_color || GOLD
      ...
      <div className="border-b" style={{ borderColor: BORDER }}>
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="font-bold text-lg truncate" style={{ color: INK }}>{series.title}</h1>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <Link href={`/educacion/estudio-biblico/${series.slug}`} target="_blank"
              className="w-9 h-9 flex items-center justify-center rounded-xl transition hover:bg-[#292E3B]"
              style={{ color: MUTED }} title="Ver en sitio">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <p className="font-bold" style={{ color: INK }}>{series.book || series.title}</p>
              ...
              {series.theme && (
                <p className="text-sm mb-2" style={{ color: MUTED }}>{series.theme}</p>
              )}
              {series.description && (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/admin/estudio-biblico/${id}/sesiones/nueva`}
              ...
              style={{ background: CARD, color: GOLD, border: `1px solid ${BORDER}` }}>
          ...
          {!sessions || sessions.length === 0 ? (
            <div className="rounded-2xl p-10 text-center" style={{ background: CARD, border: `1px solid ${BORDER}` }}>
              <p className="text-sm" style={{ color: MUTED }}>No hay sesiones todavía.</p>
              <Link href={`/admin/estudio-biblico/${id}/sesiones/nueva`}
                  ...
                  <div key={sess.id} className="flex items-center gap-4 px-5 py-4 border-b"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="font-bold text-sm truncate" style={{ color: INK }}>{sess.title}</p>
                        {!sess.is_active && (
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <Link href={`/admin/estudio-biblico/${id}/sesiones/${sess.id}`}
                        className="w-8 h-8 flex items-center justify-center rounded-lg transition hover:bg-[#292E3B]"
                        style={{ color: MUTED }}>
                        <Pencil size={13} />
                      </Link>

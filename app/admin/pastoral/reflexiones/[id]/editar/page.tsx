...
import { updatePastoralReflection } from '@/app/actions/pastoral-admin'

export default async function EditarReflexionPage({
  params,
}: {
  ...
  const { data: item } = await supabase
    ...
    .select('id, title, body, media_type, media_url, duration_seconds, week_featured')
    .eq('id', id)
    .single()
  ...
  const action = updatePastoralReflection.bind(null, id)
    ...
    <div>
      <div className="border-b" style={{ borderColor: '#0D3352' }}>
        <div className="max-w-3xl mx-auto px-4 md:px-8 py-5 flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-lg text-white">Editar reflexión</h1>
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-6">
        <form action={action} encType="multipart/form-data" className="space-y-5">
          ...
          <div>
            ...
            <select name="media_type" defaultValue={item.media_type ?? 'text'} className={field} style={fieldStyle}>
          ...
          <div>
            ...
            <input name="title" defaultValue={item.title ?? ''} placeholder="Ej: La gracia que sostiene"
              className={field} style={fieldStyle} />
          </div>

          <div>
            <label className={label} style={labelStyle}>Contenido / Reflexión</label>
            <textarea name="body" rows={6} defaultValue={item.body ?? ''} placeholder="Escribe la reflexión..."
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="media_url_external" type="url" defaultValue={item.media_url ?? ''}
                placeholder="https://..." className={field} style={fieldStyle} />
            </div>
            <div>
              <label className={label} style={labelStyle}>Duración (segundos)</label>
              <input name="duration_seconds" type="number" min="0"
                defaultValue={item.duration_seconds ?? ''} placeholder="180"
                className={field} style={fieldStyle} />
            </div>
          </div>

          <div>
            <label className={label} style={labelStyle}>Reemplazar archivo multimedia (opcional)</label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="week_featured" defaultChecked={!!item.week_featured}
